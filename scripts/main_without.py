...
def train_advent(teacher_model,student_model, trainloader, targetloader, cfg,growth_rate,init_beta):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)):
        ...
        pred_src_aux, pred_src_main = teacher_model(images_source.cuda(device))
        ...
        images, _, _, _ = batch
        pred_trg_aux, pred_trg_main = teacher_model(images.cuda(device))
        if cfg.TRAIN.MULTI_LEVEL:
        ...
        loss_kl_stu = loss1_src + 0.1 *loss2_src
        ...
        if i_iter % 500 == 0 and i_iter != 0:
            ...
            print('exp =', 'new_main_without')
            torch.save(teacher_model.state_dict(), osp.join('../../new_main_without', f'model_{i_iter}.pth'))
            torch.save(d_aux.state_dict(), osp.join('../../new_main_without', f'model_{i_iter}_D_aux.pth'))
            torch.save(d_main.state_dict(), osp.join('../../new_main_without', f'model_{i_iter}_D_main.pth'))
            torch.save(student_model.state_dict(), osp.join('../../new_main_without', f'model_{i_iter}_stu.pth'))
