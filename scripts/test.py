...
from advent.model.deeplabv2_fix import get_deeplab_v2_resnet50,get_deeplab_v2_resnet101
...
def train_advent(model, trainloader, targetloader, cfg):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)):
        ...
        prediction, attentions = model(images_source.cuda(device))
        # _, prediction = model(images_source.cuda(device))
        prediction = interp(prediction)
        loss_fun = MyLoss_correction(att_depth=3,out_channels=num_classes, patch_size=5)
        loss_src = loss_fun.forward(prediction, labels, attentions)
        # loss = loss_fun.forward(prediction, labels)
        # loss = loss_calc(prediction, labels, device)
        loss = loss_src
        ...
        images, _, _, _ = batch
        pred_trg_main, _ = model(images.cuda(device))
        ...
        # train with source
        pred_src_main = prediction.detach()
        ...
        current_losses = {'loss_src': loss_src,
        ...
        if i_iter % cfg.TRAIN.SAVE_PRED_EVERY == 0 and i_iter != 0:
            ...
            snapshot_dir = Path(cfg.TRAIN.SNAPSHOT_DIR+'111')
...
def main(config_file, exp_suffix):
    ...
    saved_state_dict = torch.load('../../new_main/model_9500.pth')
