...
from advent.model.swin_transformer import get_swin_transformer
...
RESTORE_FROM = './lpy_10_09/swin_transformer_original'
...
def main():
    ...
    # 模型加载参
    start_iter = 2000  # 从第 轮开始保存
    step = 2000  # 每隔 轮保存一次
    max_iter = 25000  # 最多保存到 轮
        ...
        if i_iter not in all_res.keys():
            ...
            for index in tqdm(range(len(testloader))):
                ...
                with torch.no_grad():
                    ...
                    pred_main = model(image.cuda(device))
    ...
    #         output1, output2 = model(Variable(image, volatile=True).to(device))
...
def load_checkpoint_for_evaluation(model, checkpoint, device):
    saved_state_dict = torch.load(checkpoint, map_location='cuda:1')
    model.load_state_dict(saved_state_dict)
    model.eval()
    model.cuda(device)
