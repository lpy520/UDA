...
#交叉熵损失通常用于分类任务，包括语义分割，用于衡量模型预测概率与真实标签的差异
def loss_calc(pred, label, device):
    ...
    label = label.long().to(device)    #将label张量转换成long型，移动到
    return cross_entropy_2d(pred, label) #计算预测结果与目标标签之间的损失
...
def per_class_iu(hist):
    # (真正例)/(真实例总数+预测实例总数-真正例)  --->得到每个类比的正确预测像素数量与并集像素总数的比值，是为了得到每个类别的预测准确度
    return np.diag(hist) / (hist.sum(1) + hist.sum(0) - np.diag(hist))
     #hist.sum(1):每一行进行求和，得到一个包含每个行的和的一维数组
     #hist.sum(0):每一列进行求和，得到一个包含每个列的和的一维数组
